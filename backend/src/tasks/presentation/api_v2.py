...
async def create_from_image_to_image(
    user: ValidateTokensBalanceDepend,
    uow: TaskUoWDepend,
    ...
    if schema.user_id != user.external_id or schema.app_bundle != user.app_bundle:
        raise HTTPException(422, detail="Invalid user_id or app_bundle")
...
async def get_task_info(_: CurrentUserDepend, task_id: UUID, uow: TaskUoWDepend):
