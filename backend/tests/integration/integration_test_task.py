...
create_dto = TaskCreateImageDTO(user_prompt="test prompt", user_id="test", app_bundle="prompt", size=TaskResultSize.square)


@pytest.mark.asyncio(loop_scope="session")
async def test_create_and_wait_for_result(client: httpx.AsyncClient, fake_task_source: ITaskSourceClient, mock_image: BytesIO):
    ...
    async with override_dependencies({get_openai_adapter: lambda: fake_task_source}):
        while True:
            ...
            if body.get("items"):
                break
    assert body["items"][0].get('result_url') and body["items"][0]["result_url"].endswith(f"/api/task/{body['id']}/result"), "Invalid task result_url received"
...
async def _create(client: httpx.AsyncClient, task_source: ITaskSourceClient, image: BytesIO) -> dict:
