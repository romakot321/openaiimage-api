...
class TaskService:
    ...
    async def _send(self, task_id: UUID, method: Coroutine):
        ...
        await self.task_repository.create_items(TaskItem(task_id=task_id, result_url=result))

    async def send_txt2img(self, task_id: UUID, schema: TaskCreateSchema):
        prompt = await self.build_prompt(schema)
        request = ExternalText2ImageTaskSchema(prompt=prompt, size=schema.size, quality=schema.quality)
    ...
    async def send_img2img(self, task_id: UUID, schema: TaskCreateSchema, image: BytesIO):
        ...
        request = ExternalImage2ImageTaskSchema(prompt=prompt, size=schema.size, image=image, quality=schema.quality)
